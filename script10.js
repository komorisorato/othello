    ...
    if (isPass()) {
        toastr.remove();
         Swal.fire({
                  ...
                  icon: 'error',  // アイコンの種類（'success', 'error', 'warning', 'info', 'question'）
                  confirmButtonText: '確認',
                  allowOutsideClick: false,  // ダイアログ外のクリックで閉じない
...
function initializeEvent() {
    ...
    // 初期値設定
    changeOwner(getTargetSquare(4, 5));
    changeOwner(getTargetSquare(4, 4));
    changeOwner(getTargetSquare(5, 4));
    changeOwner(getTargetSquare(5, 5));
...
function getPosOppositeLower(row, col) {
    ...
    // 対向先の有無を判定する
    for (targetRow++; targetRow <= 99; targetRow++) {
        // マスの状態を取得する
        let squareType = getSquareStatus(targetRow, targetCol);
...
function getPosOppositeRight(row, col) {
    ...
    // 対向先の有無を判定する
    for (targetCol++; targetCol <= 99; targetCol++) {
        // マスの状態を取得する
        let squareType = getSquareStatus(targetRow, targetCol);
