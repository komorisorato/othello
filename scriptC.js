 ...
 *****************************************/
 function cpuTurn() {
    if (isOddTurn) return; // CPUは後攻（白）なので、先攻のときは動かない
    ...
    if (availableMoves.length === 0) {
        ...
        // 両者がパスしたらゲーム終了
        if (isPass()) {
            toastEndMessage("選択できるマスがなくなりました。ゲーム終了");
            return;
        }
        changeTurn();
        setTimeout(() => {
    ...
    let chosenSquare = availableMoves[randomIndex];
    changeOwner($(chosenSquare));
    ...
    // CPUの手番が終了し、次はプレイヤーの番
    setTimeout(() => {
        toastr.remove();
        let countBlack = $("[data-owner=black]").length;
        let countWhite = $("[data-owner=white]").length;
        toastr.info(`黒の番<br>現在の状況 <br>黒:${countBlack} <br>白:${countWhite}<br>※ダブルクリックで非表示`);
    }, 10);
}

// 変更：プレイヤーのターン終了後にCPUを呼び出す
function clickSquareEvent() {
    let square = $(this);
    ...
    let countBlack = $("[data-owner=black]").length;
    let countWhite = $("[data-owner=white]").length;
    toastr.info(`白の番<br>現在の状況 <br>黒:${countBlack} <br>白:${countWhite}<br>※ダブルクリックで非表示`);
 ...
 */
 function changeTurn() {
    ...
    console.log("選択可能なマス:", $(".square.can-select").length);
